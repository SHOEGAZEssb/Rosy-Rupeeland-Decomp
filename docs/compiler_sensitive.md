...
| `GraphicsTransferQueue_Reset` | MWCC reverses the retail initialization of the loop counter and retained queue pointer. Two backward loop branches are hand encoded to avoid inline-assembly fallthrough branches. | Clear all 128 entries before rebuilding their `nextOrFreeNext` links, reset both active endpoints and `queuedCount`, and leave the final descriptor's next/free link null as established by its clear. Reset discards pending metadata without executing transfers. |
| `GraphicsTransferEntry_Clear`, `GraphicsTransferQueue_Enqueue` | Declaring the stored transfer-kind word or enqueue parameter as a C++ enum changes otherwise exact instruction selection. | Keep the ABI and entry storage `u32`, use `GraphicsTransferKind` only to define the three verified object-VRAM kind constants, and reject speculative values at the consumer with the retail default branch. |
| `GraphicsIndexedChainPool_AllocateChain` | MWCC assigns different registers to all four pool endpoints and the returned root; the matching loop uses a hand-encoded backward branch. | Require a positive requested count, append only the first descriptor as an active root, store `bindingMode` only on that root, chain later descriptors through `chainNext`, and increment `allocatedEntryCount` for every descriptor actually obtained. |
| `GraphicsIndexedChainPool_ReleaseChain` | Equivalent C produces a smaller function by changing register lifetimes and combining the free-list loop. The matching backward branch is hand encoded. | Detach the root from the active list before recycling it and its entire chain, repair both endpoints, clear `previousOrFreePrevious`, `nextOrFreeNext`, and `chainNext` as shown, and decrement `allocatedEntryCount` once per descriptor. |
