# Compiler-sensitive functions and the native recompilation
...
an expression whose ordering deserves explicit review during native recompilation.
...
fallback or when the compiler-dependent form exposes a recompilation constraint.
...
expanded-viewport bounds, and caller-selected culling flag. A native recompilation
...
and the ordering of flag set/clear masks. A native recompilation should call the typed
...
rounding, and the renderer's mode/advance/spacing arguments. A native recompilation
