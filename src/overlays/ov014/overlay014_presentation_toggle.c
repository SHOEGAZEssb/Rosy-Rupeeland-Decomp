 ...
 * +0x0C member, call Overlay000_PopulatePresentation on +0x88 and set state flag bit 1 at
...
void func_ov014_021fd2f8(void *state, s32 forceDisabled)
{
    ...
    if (forceDisabled == 0) {
        void *entry = Overlay000_GetActiveMetadata(*(void **)((u8 *)state + 0x78));
        record = *(void **)((u8 *)entry + 0x0c);
    }

    if (record != 0) {
        Overlay000_PopulatePresentation(*(void **)((u8 *)state + 0x88), record);
        *(u32 *)((u8 *)state + 0x4c) |= 2;
    } else {
