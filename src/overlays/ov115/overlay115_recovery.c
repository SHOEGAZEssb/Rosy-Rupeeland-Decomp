/*
 * Recovered overlay 115 compact interpreter-data subsystem.
 *
...
extern unsigned char data_ov115_0221d72c[], data_ov115_0221d740[];
extern unsigned char data_ov115_0221efc9[], data_ov115_02227664[];

/* Register this overlay's category-one descriptors only for load phase zero. */
void func_ov115_0221d6c0(s32 phase)
{
    if (phase != 0) return;
    ActorInteractionRuntime_Init();
    ActorDescriptorBatch_RegisterAndSpawnCategory1(data_ov115_02227664,
        3, data_ov115_0221d740);
    ActorInteractionRuntime_NoOp();
    GamePhaseRuntime_SetGlobalValue30F8(data_ov115_0221d72c);
    ActorDescriptorBatch_SetCategory1Callback(data_ov115_0221efc9);
}

/* Exact initialized records and zero-initialized work fields used by the overlay. */
extern unsigned char data_ov115_0221d720[];
extern unsigned char data_ov115_02227660[];
