...
extern "C" void func_ov025_02200498(void *scene, s32 layout)
{
    ...
    for (i = 0; i < 3; ++i) {
        ...
        SpriteMotionController_SetPosition(controller, 0xd4, (s16)positions[i]);
        SpriteMotionController_SetAnimation(controller, 0x20 + i * 2);
        SpriteMotionController_Show(controller);
    }
}
