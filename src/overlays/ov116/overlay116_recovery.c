/*
 * Recovered overlay 116 compact interpreter-data subsystem.
 *
...
extern unsigned char data_ov116_0221d720[], data_ov116_0221d734[];
extern unsigned char data_ov116_0221fe31[];
extern unsigned char data_ov116_02229de4[];

/* Register this overlay's category-one descriptors only for load phase zero. */
void func_ov116_0221d6c0(s32 phase)
{
    if (phase != 0) return;
    ActorInteractionRuntime_Init();
    ActorDescriptorBatch_RegisterAndSpawnCategory1(data_ov116_02229de4,
        3, data_ov116_0221d734);
    ActorInteractionRuntime_NoOp();
    GamePhaseRuntime_SetGlobalValue30F8(data_ov116_0221d720);
    ActorDescriptorBatch_SetCategory1Callback(data_ov116_0221fe31);
}

/* Exact initialized records and zero-initialized work fields used by the overlay. */
extern unsigned char data_ov116_0221d720[];
extern unsigned char data_ov116_02229de0[];
