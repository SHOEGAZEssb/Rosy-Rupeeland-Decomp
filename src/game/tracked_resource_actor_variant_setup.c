...
/* Recovered record and transform setup for the second tracked-resource actor variant. */
...
/*
 * Inputs are a variant actor, its record, a transform-related argument, and an
 * unused fourth value. Stores the record, derives and copies a vector into
 * field 0x38, applies signed record byte 0x15, clamps a magnitude-derived
 * divisor to at least 4, divides the vector by it, and scales by record halfword
 * 0x0A shifted four bits. It then
 * forces field 0x44 to 0x1800, sets flag 0x800000 at 0x14 and flag 0x2000 at
