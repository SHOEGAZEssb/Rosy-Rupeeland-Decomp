...
{
    u32 inventoryId = GamePhaseScriptVm_Pop(&self->base);
    GamePhaseScriptVm_StoreResultAndUpdateCondition(&self->base, InventoryManager_ContainsId(data_021e9ac0, inventoryId) != 0);
    return 0;
}
...
s32 GamePhaseActorScriptVm_SecondaryTableContainsValue(GamePhaseActorScriptVm *self)
{
    ...
    GamePhaseScriptVm_StoreResultAndUpdateCondition(&self->base, func_0206f9d4(data_021e9e00, tableValue) != 0);
    return 0;
}
