 ...
 * active, start one from records through ActorDerivedType1_StartRecord. Other
 * active descriptor types are unchanged. Returns no value; record start may
 * allocate or play audio.
 */
void ActorDerivedType1_StartRecordOrHandleType6D66(void *self, void *records)
{
    u8 *actor = (u8 *)self;
 ...
 * ActorDerivedType1_ApplyActiveRecord. Returns no value; teardown, object, and
 * apply calls alter ownership.
 */
void ActorDerivedType1_BindExternalRecordObject(void *self, void *object)
{
    u8 *actor = (u8 *)self;
 ...
 * down the active descriptor, then dispatch vector/mode through
 * Actor_ApplyMotionImpulse and finalize with Type1Actor_TryEnterFailureState.
 * Returns no value; helper calls may update actor presentation, ownership, and
 * motion state.
 */
void ActorDerivedType1_TryApplyMotionImpulse(void *self, const void *vector, s32 mode)
{
    u8 *actor = (u8 *)self;
    u8 *object;
...
/*
 * Tear down the active descriptor and dispatch vector through
 * Actor_ApplyMotionImpulse with mode one. Returns no value; both helpers change
 * actor-owned state.
 */
void ActorDerivedType1_TeardownRecordAndApplyMotionImpulse(void *self, const void *vector)
{
