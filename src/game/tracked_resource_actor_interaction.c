...
extern "C" {
#endif
extern void ActorDerivedType1_DispatchInteractionAmount(void *target, s32 value, void *source);
extern void Type7Actor_NoopTrackedResourceInteraction(void *target, s32 value, const void *record,
                          u32 argument);
#ifdef __cplusplus
}
#endif
 ...
 * halfword 0x0C and the originating actor, whose record damage handling reads.
 * For target type 7, call Type7Actor_NoopTrackedResourceInteraction with signed record
...
void TrackedResourceActor_DispatchTargetInteraction(void *actor, void *target, u32 unused, u32 argument)
{
    ...
    if (FIELD(u8, target, 0x4d) == 1) {
        ActorDerivedType1_DispatchInteractionAmount(target, FIELD(s16, record, 0x0c), actor);
    } else if (FIELD(u8, target, 0x4d) == 7) {
        Type7Actor_NoopTrackedResourceInteraction(target, FIELD(s16, record, 0x0e), record, argument);
    }
}
