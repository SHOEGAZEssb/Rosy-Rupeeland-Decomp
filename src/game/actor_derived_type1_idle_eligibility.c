 ...
 * bit 0x20000 are clear; ActorDerivedType1_HasBlockingStateFlags is false;
 * signed cooldown +0x268 is zero; and owned objects +0x274/+0x278 are null.
 * Otherwise return zero. No fields change;
 * the two predicates and virtual query may inspect wider engine state.
 */
s32 ActorDerivedType1_IsIdleEligible(void *self)
{
    u8 *actor = (u8 *)self;
    ...
    if (Actor_IsAtCachedTerrainHeight(actor) == 0 ||
        ...
        return 0;
    if (ActorDerivedType1_HasBlockingStateFlags(actor) != 0 || *(s16 *)(actor + 0x268) != 0 ||
