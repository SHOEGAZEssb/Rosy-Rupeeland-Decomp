 ...
 * component as the VM result, and return zero. The table's precise SDK identity
 * is unconfirmed.
 */
 ...
 * by 256 with truncation toward zero, store the result as the VM result, and
 * return zero.
 */
s32 GamePhaseActorScriptVm_ComputeAngle(GamePhaseActorScriptVm *self)
{
    fx32 secondComponentFx = (s32)GamePhaseScriptVm_Pop(&self->base) << 12;
    fx32 firstComponentFx = (s32)GamePhaseScriptVm_Pop(&self->base) << 12;
    s32 angleFx16 = (s16)func_020ae024(firstComponentFx, secondComponentFx);
    GamePhaseScriptVm_StoreResultAndUpdateCondition(
        &self->base, (angleFx16 + ((u32)angleFx16 >> 24)) >> 8);
    return 0;
}
...
s32 GamePhaseActorScriptVm_GetVectorX(GamePhaseActorScriptVm *self)
{
    ...
    VecFx32Object_InitCopy(&actorPosition, (VecFx32Object *)((u8 *)self->actor + 0x18));
    GamePhaseScriptVm_StoreResultAndUpdateCondition(&self->base, actorPosition.value.x >> 12);
...
s32 GamePhaseActorScriptVm_GetVectorY(GamePhaseActorScriptVm *self)
{
    VecFx32Object actorPosition;
    VecFx32Object_InitCopy(&actorPosition, (VecFx32Object *)((u8 *)self->actor + 0x18));
    GamePhaseScriptVm_StoreResultAndUpdateCondition(&self->base, actorPosition.value.y >> 12);
    VecFx32Object_Destroy(&actorPosition);
    return 0;
}
...
s32 GamePhaseActorScriptVm_GetVectorZ(GamePhaseActorScriptVm *self)
{
    VecFx32Object actorPosition;
    VecFx32Object_InitCopy(&actorPosition, (VecFx32Object *)((u8 *)self->actor + 0x18));
    GamePhaseScriptVm_StoreResultAndUpdateCondition(&self->base, actorPosition.value.z >> 12);
    VecFx32Object_Destroy(&actorPosition);
    return 0;
}
